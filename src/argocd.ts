...
export async function newerRevision(app: Application): Promise<string> {
    ...
    const newTargetRevision = index.entries[app.spec.source.chart][0].version
      ...
      if (!compareVersions.validate(newTargetRevision) || !compareVersions.validate(app.spec.source.targetRevision)) {
        return ''
      }

      // Continue only if a new version is available
      if (compareVersions.compare(newTargetRevision, app.spec.source.targetRevision, '<=')) {
        return ''
      }
...
const HelmChartRepositories: Record<string, Promise<HelmChartRepositoryIndex>> = {}
...
export async function helmRepoIndex(url: string): Promise<HelmChartRepositoryIndex> {
